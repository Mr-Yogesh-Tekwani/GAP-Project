...
class MediaViewController: UIViewController, WKNavigationDelegate {
    ...
    func saveComment(_ comment: String) {
            ...
            DispatchQueue.main.async {
                let tvc = ChapterViewController()
                tvc.tableView.reloadData()
                print("Received comment: \(comment)")
                print("MVC 2 Username = ", self.username)
                let url = saveJounalUrl
                let check = saveJournalProgress(url: url!, username: self.username, chpName: self.chpName, comment: comment, level: 1, completion: { ans in
                    print(ans)
                    self.delegate?.commentSaved()
                })
                let url2 = getJournalUrl(UserName: self.username ?? "nothing")
                getJournalDetails(url: url2)
            }
        }
